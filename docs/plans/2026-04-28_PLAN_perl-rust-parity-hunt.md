...
| | |
|---|---|
| **Status** | **All four phases COMPLETE on 2026-04-28, plus extended harness.** Phase 1+2: 39 fixture-based tests, 3 unintentional regressions (F1–F3, HIGH). Phase 3 (initial): 50 `proptest` cases on gzipped synthetic FASTQ, 0 additional bugs after harness exposed P3-F1 + P3-F2. Phase 3+ (extended): 250 cases across 11 flag paths (8 SE + 3 PE), 0 new bugs. Phase 4: 213 fuzzed cases over 180s, 0 divergences. **Total: 463 differential runs, 5 findings.** All 5 findings are in CLI/plumbing code; the algorithmic core is byte-faithful |
