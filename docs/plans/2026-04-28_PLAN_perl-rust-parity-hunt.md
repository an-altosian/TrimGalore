...
| | |
|---|---|
| **Status** | **All four phases COMPLETE on 2026-04-28, plus two extension waves.** Phase 1+2: 39 fixture-based tests, 3 unintentional regressions (F1–F3, HIGH). Phase 3 (initial): 50 cases default-SE, P3-F1 + P3-F2 surfaced. Phase 3+ (extended): 250 cases across 11 flag paths, 0 new bugs. Phase 3++ (specialty + multi-pair): 140 cases across 8 more flag paths (multi-pair PE, --clock, --implicon, --demux, --rrbs --non_directional, --hardtrim3), exposed P3-F3 (--clock/--implicon imply --paired in v2.x). Phase 4: 213 fuzz cases. **Total: 692 differential runs across 19 flag paths under proptest + 31 distinct paths between proptest and fixture-based methods.** 6 findings total — 3 HIGH (F1/F2/F3 in Rust), 3 LOW–MEDIUM (P3-F1 Perl-side / P3-F2 plain-input ext / P3-F3 implies-paired) |
